...
tape( 'if provided `+infinity` for `x` and a finite `k`, the function returns `0`', opts, function test( t ) {
	var y = pdf( PINF, 1.0 );
...
tape( 'the function returns `0` for all `x < 0`', opts, function test( t ) {
	var x;
	var y;
	var i;
...
tape( 'the function evaluates the pdf for `x` given degrees of freedom `k`', opts, function test( t ) {
	...
	for ( i = 0; i < x.length; i++ ) {
		...
		if ( y === expected[i] ) {
			...
			t.ok( isAlmostSameValue( y, expected[i], 60 ), 'within tolerance. x: '+x[ i ]+'. k: '+k[i]+'. y: '+y+'. E: '+expected[ i ]+'.' );
		}
	}
	t.end();
});
